...
我总算搞清楚了JSThesis 5.5.3∗3 和你给我的那个ṽar构造的理由。我还以为要算Lan。在STLC里面更清楚:
这个 ṽar A ∈ 𝔾 要符合三个spec, (1) j* (ṽar A) = tm A, (在dt里面sigma type之后realignment就可以做到)，
(2) i*(ṽar A) = Var A ∈ Pr(𝒜). 
这是为了Hom(ṽar A, ⬤ T) ≅ Hom(ṽar A, i∗i* T) ≅ Hom(i* ṽar A, i* T) ≅ Hom(Var A, i* T) 
就是标准的Hofmann的HOAS interpertation了。所以只要类似(ṽar A → -) 且后者在closed subuniverse里面，就能做到标准的HOAS了
(3) 就是你说的injection，其实也就是JS说的representable nat transform。
但我还没太想清楚这个Pr(𝒜)内的交换图怎么lift到𝔾内,成为𝔾内ṽar A的specification.
