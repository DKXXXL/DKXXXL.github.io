...
Modern way of dealing with syntax is to use categorical syntax instead of the real syntax. 

For example, for STLC, using Coq's Inductive type to define syntax is the real syntax, but using Free CCC instead is the categorical syntax.

The issue here is that --- isn't that the semantic? Do we have the initial object in the semantic category as syntax? 

Lawvere suggested otherwise about the definition of semantic --- once we have a given category as syntax (say translating a theory into a category and consider it as a syntax), then we consider *a functor from this category to another* as a model (using the codomain as the interpretation). Simply because model is a proof-relevant concept --- two functors come to same category should be considered as two different models. Then model homomorphism is the natural transformation between functors. This is functorial semantic.
...
3. To summarize --
   ...
   2. If a system is described from the beginning in a dynamic style (very operational) without clear idea of contextual equivalence from the beginning, then cat semantic is a bit hard to get for them
4. The biggest problem for cat semantic:
   ...
   2. for example, `(λ x. x)y` is equal to `y`, for cat semantic, a normalizer return `(λ x. x)y` is ok, but for a practical PList, it is not
   1. this problem is solvable, but either in a very sophisticated way (2-cat, P-cat) or in a non-cat way (introduce concrete normal form as data)
5. This doesn't cover topos stuff, just about the discussion of categorical semantic/syntax.